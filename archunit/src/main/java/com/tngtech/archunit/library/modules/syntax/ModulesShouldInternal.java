...
class ModulesShouldInternal<DESCRIPTOR extends ArchModule.Descriptor> implements ModulesShould<DESCRIPTOR> {
    ...
    private static class ModulesRuleInternal<DESCRIPTOR extends ArchModule.Descriptor> implements ModulesRule {
        ...
        public ModulesRule as(String newDescription) {
            return new ModulesRuleInternal<>(createRule, createCondition, rule -> modifyRule.apply(rule).as(newDescription), __ -> true);
        }

        @Override
        public ModulesRule because(String reason) {
            return new ModulesRuleInternal<>(createRule, createCondition, rule -> modifyRule.apply(rule).because(reason), __ -> true);
        }

        @Override
        public ModulesRule allowEmptyShould(boolean allowEmptyShould) {
            return new ModulesRuleInternal<>(createRule, createCondition, rule -> modifyRule.apply(rule).allowEmptyShould(allowEmptyShould), __ -> true);
        }
