            ...
            public void assertNoMissingDependencies() {
                if (!Iterables.isEmpty(missingDependencies)) {
                    throw new AssertionError("Could not find expected dependencies:" + lineSeparator()
                            ...
                            + descriptionsOf(actualDependencies).stream().collect(joining(lineSeparator())));
                }
            }

            private List<String> descriptionsOf(Collection<? extends HasDescription> haveDescriptions) {
                return haveDescriptions.stream().map(HasDescription::getDescription).collect(toList());
            }
